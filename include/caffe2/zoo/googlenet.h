...
class GoogleNetModel : public ModelUtil {
 public:
  ...
  OperatorDef *AddFcOps(const std::string &input, const std::string &output, int in_size, int out_size, bool relu, float dropout = 0.5) {
  ...
  OperatorDef *AddFirst(const std::string &prefix, const std::string &input, int in_size, int out_size) {
    auto output = "conv" + prefix + "/";
    std::string layer = input;
    layer = AddConvOps(layer, output + "7x7_s2", in_size, out_size, 2, 3, 7)->output(0);
    layer = predict_.AddMaxPoolOp(layer, "pool" + prefix + "/3x3_s2", 2, 0, 3)->output(0);
    return predict_.AddLrnOp(layer, "pool1/norm1", 5, 0.0001, 0.75, 1);
  }

  OperatorDef *AddSecond(const std::string &prefix, const std::string &input, int in_size, int out_size) {
    auto output = "conv" + prefix + "/3x3";
    std::string layer = input;
    layer = AddConvOps(layer, output + "_reduce", in_size, out_size / 3, 1, 0, 1)->output(0);
  ...
  OperatorDef *AddInception(const std::string &prefix, const std::string &input, std::vector<int> sizes) {
    ...
    for (int i = 0, kernel = 1; i < 3; i++, kernel += 2) {
      ...
      if (i) {
        layer = AddConvOps(input, b + "_reduce", sizes[0], sizes[kernel - 1], 1, 0, 1)->output(0);
      }
      layers.push_back(AddConvOps(layer, b, sizes[kernel - 1], sizes[kernel], 1, i, kernel)->output(0));
    }
    layer = predict_.AddMaxPoolOp(input, output + "pool", 1, 1, 3)->output(0);
    layers.push_back(AddConvOps(layer, layer + "_proj", sizes[0], sizes[6], 1, 0, 1)->output(0));
    return predict_.AddConcatOp(layers, output + "output");
  }

  OperatorDef *AddSide(const std::string &prefix, const std::string &input, std::vector<int> sizes, int out_size) {
    auto output = "loss" + prefix + "/";
    std::string layer = input;
    layer = predict_.AddAveragePoolOp(layer, output + "ave_pool", 3, 0, 5)->output(0);
    layer = AddConvOps(layer, output + "conv", sizes[0], sizes[1], 1, 0, 1)->output(0);
    layer = AddFcOps(layer, output + "fc", sizes[2], sizes[3], true, 0.7)->output(0);
  ...
  OperatorDef *AddEnd(const std::string &prefix, const std::string &input, int in_size, int out_size) {
    auto output = "loss" + prefix + "/";
    std::string layer = input;
    layer = predict_.AddAveragePoolOp(layer, "pool5/7x7_s1", 1, 0, 7)->output(0);
  ...
  void Add(int out_size = 1000, bool train = false) {
    ...
    layer = AddInception("3a", layer, { 192, 64, 96, 128, 16, 32, 32 })->output(0);
    layer = AddInception("3b", layer, { 256, 128, 128, 192, 32, 96, 64 })->output(0);
    layer = AddPool("3", layer)->output(0);
    layer = AddInception("4a", layer, { 480, 192, 96, 208, 16, 48, 64 })->output(0);
    if (train) {
      auto last = layer;
    ...
    layer = AddInception("4b", layer, { 512, 160, 112, 224, 24, 64, 64 })->output(0);
    layer = AddInception("4c", layer, { 512, 128, 128, 256, 24, 64, 64 })->output(0);
    layer = AddInception("4d", layer, { 512, 112, 144, 288, 32, 64, 64 })->output(0);
    if (train) {
      auto last = layer;
    ...
    layer = AddInception("4e", layer, { 528, 256, 160, 320, 32, 128, 128 })->output(0);
    layer = AddPool("4", layer)->output(0);
    layer = AddInception("5a", layer, { 832, 256, 160, 320, 32, 128, 128 })->output(0);
    layer = AddInception("5b", layer, { 832, 384, 192, 384, 48, 128, 128 })->output(0);
