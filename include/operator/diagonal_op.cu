#include "caffe2/operators/diagonal_op.h"
...
__global__ void DiagonalKernel(const int N, const float ratio, const float* Xdata, float* Ydata, bool* maskdata) {
...
bool DiagonalOp<float, CUDAContext>::RunOnDevice() {
  ...
  if (is_test_) {
    ...
    CAFFE_ENFORCE(X.data<float>() != Ydata, "In-place GPU Diagonal is broken");
    CURAND_ENFORCE(
        curandGenerateUniform(context_.curand_generator(), Ydata, X.size()));
    DiagonalKernel<<<CAFFE_GET_BLOCKS(X.size()), CAFFE_CUDA_NUM_THREADS,
...
__global__ void DiagonalGradientKernel(const int N, const float* dYdata, const bool* maskdata, const float scale, float* dXdata) {
  CUDA_1D_KERNEL_LOOP(i, N) {
...
bool DiagonalGradientOp<float, CUDAContext>::RunOnDevice() {
  ...
  if (is_test_) {
    ...
    DiagonalGradientKernel<<<CAFFE_GET_BLOCKS(dY.size()),
...
REGISTER_CUDA_OPERATOR(DiagonalGrad, DiagonalGradientOp<float, CUDAContext>);
