#include "caffe2/operators/mean_stdev_op.h"
...
__global__ void MeanStdevKernel(const int N, const float ratio, const float* Xdata, float* Ydata, bool* maskdata) {
...
bool MeanStdevOp<float, CUDAContext>::RunOnDevice() {
  ...
  if (is_test_) {
    ...
    CAFFE_ENFORCE(X.data<float>() != Ydata, "In-place GPU MeanStdev is broken");
    CURAND_ENFORCE(
        curandGenerateUniform(context_.curand_generator(), Ydata, X.size()));
    MeanStdevKernel<<<CAFFE_GET_BLOCKS(X.size()), CAFFE_CUDA_NUM_THREADS,
...
__global__ void MeanStdevGradientKernel(const int N, const float* dYdata, const bool* maskdata, const float scale, float* dXdata) {
  CUDA_1D_KERNEL_LOOP(i, N) {
...
template <>
bool MeanStdevGradientOp<float, CUDAContext>::RunOnDevice() {
  auto& dY = Input(0);
  auto& mask = Input(1);
  auto* dX = Output(0);
  ...
  if (is_test_) {
    ...
    MeanStdevGradientKernel<<<CAFFE_GET_BLOCKS(dY.size()),
...
REGISTER_CUDA_OPERATOR(MeanStdevGrad, MeanStdevGradientOp<float, CUDAContext>);
